...
export class EventManager<T = unknown> {
	...
	public bind(
		...
		try {
			...
			if (bindings.length > 0) this.bindings.set(loaded, bindings);
		} catch (error) {
	...
	private bindMap(
		...
		for (const [event, definition] of Object.entries(map)) {
			...
			for (const handlerDefinition of handlers) {
				...
				if (once) target.once(event, bound);
				else target.on(event, bound);

				bindings.push({ event, handler: bound, once, target });
			}
		}
	}
